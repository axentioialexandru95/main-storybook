...
export const CanvasTiles = forwardRef<CanvasTilesRef, CanvasTilesProps>(({
  ...
  useEffect(() => {
    ...
    const resizeCanvas = () => {
      ...
      tilesData.current = Array(rows).fill(0).map((_, i) => 
    ...
    const drawTiles = (timestamp: number) => {
      ...
      if (hoverPosition.current && fadeInProgress.current > 0.9) {
        ...
        const gradient = ctx.createRadialGradient(
          hoverPosition.current.x, hoverPosition.current.y, 0,
          hoverPosition.current.x, hoverPosition.current.y, animatedRadius
        )
        ...
        gradient.addColorStop(0, `rgba(5, 5, 50, ${animatedOpacity})`)
